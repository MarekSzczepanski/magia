...
const HeroBar = styled.div`
  ...
  padding: 0 3vw;
  border-top: 0.2vw solid #1a1e23;
  border-bottom: 0.2vw solid #1a1e23;
  background-color: #ff0420;
`;
...
const AnimationWrap = styled.div`
  ...
  animation: ${(props) => props.animation} 20s linear infinite alternate,
    ${animationTo('opacity: .2')} 4s ease-in infinite alternate;
`;
...
const Hero = ({ h1, heroBarHeadings, heroBarTransform, heroBarTransformMobile }) => (
  <Wrap>
    ...
    <HeroBar>
      <HeroBarWrap
        animation={animationTo(`transform: translateX(${heroBarTransform})`)}
        animationPortrait={animationTo(`transform: translateX(${heroBarTransformMobile})`)}
      >
    ...
    {allStars.map((star, i) => {
      return (
        <AnimationWrap
          ...
          top={star.top}
          left={star.left}
          animation={animationTo(star.animation)}
          key={i}
        >
          <StaticImage
