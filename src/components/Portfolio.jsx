...
const BoxBox = styled.div`
    ...
    @media (orientation: landscape) {
        ...
        margin-left: ${props => props.boxNumber > props.slidesSlid + 1 ? 20 : 0}vw;
        transform: translateX(${props => props.slidesSlid > props.boxNumber ? -20 : 0}vw);
        transition: box-shadow .3s ease-out, transform 1.5s ease-out, margin 1.5s ease-out;
        &:hover {
...
const LogoParent = styled.div`
    ...
    width: 9vw;
    height: 9vw;
    transform: translate(${props => props.slidesSlid > props.boxNumber ? -20 : 0}vw, -100%) rotate(25deg);
...
const Chevron = styled.div`
    ...
    &.right-chevron {
        ...
        pointer-events: ${props => props.slidesSlid + 2 < Boxes.length && !props.isSliding ? 'auto' : 'none'};
    }
    &:hover {
        border-color: #ff0420;
    }
    @media (orientation: portrait) {
        ...
        &.right-chevron {
            ...
            pointer-events: ${props => props.slidesSlid + 1 < Boxes.length && !props.isSliding? 'auto' : 'none'};
        }
    }
`
...
const Portfolio = () => { 
    ...
    const changeSlide = (e) => {
        ...
        if (dimensions.width < dimensions.height) setSliderTransform(sliderTransform + 100 * transformDirection);
        else setSliderTransform(sliderTransform + 40.5 * transformDirection);

        setIsSliding(true);
        setTimeout(() => { setIsSliding(false) }, 1500);
    }
    
    return (
        <Wrap>
            <H2>Nasze Realizacje</H2>
            <Chevron className='left-chevron' onClick={changeSlide} isActive={slidesSlid} isSliding={isSliding}></Chevron>
            <Container transform={sliderTransform} onTouchStart={changeSlide} onTouchMove={changeSlide}>
                {Boxes.map((project, i) => {
                    return <Box key={i}>
                        <H3 color={project.color} slidesSlid={slidesSlid} boxNumber={i}>{project.heading}</H3>
                        <BoxBox color={project.color} slidesSlid={slidesSlid} boxNumber={i} isSliding={isSliding}>
                            <ImageParent>
                                <a href={project.link} rel='noreferrer noopener' target='_blank'>{project.image}</a>
                            </ImageParent>
                        </BoxBox>
            ...
            <Chevron className='right-chevron' data-chevron='right' onClick={changeSlide} slidesSlid={slidesSlid} isSliding={isSliding}></Chevron>
            {gems.map((gem, i) => {
                return <GemContainer 
                top={gem.top} 
                left={gem.left} 
